...
const int FastCharStream::maxRewindSize = 255*2;
  ...
  void FastCharStream::readChar(char &c) {
	/*try{*/
		int r = input->read();
		if ( r == -1 )
			input = NULL;
 ...
 //   printf("getnext\n");
    ...
    {
      //_CLTHROWA(CL_ERR_IO,"warning : FileReader.GetNext : Read char over EOS.");
    }
...
//      printf("UnGet \n");
  ...
  int FastCharStream::Column() const {
	return col;
  }

  int FastCharStream::Line() const {
	return line;
  }
